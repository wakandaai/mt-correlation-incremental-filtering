...
def plot_scatter_correlations(df, language, split, output_dir):
    ...
    # QE vs chrF++
    axes[0].scatter(df['ssa_comet_qe_score'], df['chrf_score'], alpha=0.5, s=20)
    r_pearson = stats.pearsonr(df['ssa_comet_qe_score'], df['chrf_score'])[0]
    r_spearman = stats.spearmanr(df['ssa_comet_qe_score'], df['chrf_score'])[0]
    
    # Add regression line
    z = np.polyfit(df['ssa_comet_qe_score'], df['chrf_score'], 1)
    p = np.poly1d(z)
    axes[0].plot(df['ssa_comet_qe_score'], p(df['ssa_comet_qe_score']), 
    ...
    # QE vs BLEU
    axes[1].scatter(df['ssa_comet_qe_score'], df['bleu_score'], alpha=0.5, s=20)
    r_pearson = stats.pearsonr(df['ssa_comet_qe_score'], df['bleu_score'])[0]
    r_spearman = stats.spearmanr(df['ssa_comet_qe_score'], df['bleu_score'])[0]
    
    # Add regression line
    z = np.polyfit(df['ssa_comet_qe_score'], df['bleu_score'], 1)
    p = np.poly1d(z)
    axes[1].plot(df['ssa_comet_qe_score'], p(df['ssa_comet_qe_score']), 
                "r--", alpha=0.8, linewidth=2)
